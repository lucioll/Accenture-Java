...
function refreshGameView(_url) {
    $.ajax({
        ...
        success: function (data) {
            ...
            var gamePlayerID =  getParameterByName("gp");
            var i = 0;
            for(var call of data.gamePlayers) {
                if (call['id'] == gamePlayerID) {
                    break;
                }
                i += 1;
            }
            gamePlayerData = gameViewData.gamePlayers[i];
            /*
            var list_players = [];
            gamePlayerData.map(function (element) {
                list_players.push(element.player);
            })

            // console.log(gamePlayerData);
            console.log(list_players[0]);
            console.log(list_players[1]);
            createTable(list_players[0],1);
            createTable(list_players[1],2);
            */


            $('#gameStateBlock').html('<span class="gameStateLabel">TURN: </span><span class="gameStateLabelBig">' + getTurn(gameViewData) + '</span><span class="gameStateLabel"> ACTION REQUIRED: </span><span class="gameStateLabelBig">' + gamePlayerData.gameState + '</span>');
...
function createTable(player, num_player) {
    var prova = 0;
    var l = 0;
    var gridLabel;
    var gridId;
    if (num_player == 1) {
        console.log("SOY EL PLAYER1");
        gridLabel = $('<p class="gridLabel">Self grid</p>');
        gridId = "#p1Grid";
    } else {
        console.log("SOY EL PLAYER2");
